...
impl<'a> MgenTag {
    ...
    pub fn get_tag_elements(data: &'a ElementRef<'a>) -> RawKumaResult<Vec<ElementRef<'a>>> {
        ...
        let elements: Vec<ElementRef<'a>> = handle_rawkuma_result!(Self::get_mgen_element(&data))
            .select(&selector)
            .collect();
...
impl<'a> FromElementRef<'a> for MgenTag {
        ...
        Self: Sized,
    {
        ...
        RawKumaResult::Ok(handle_other_error!(MgenTagBuilder::default()
            .name(name)
            .url(url)
            .build()))
    }
